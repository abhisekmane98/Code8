//  in this program error is given by display functioin 
//     because here we use n which is not defined we used so not specific length of array
//     so we must give value to n or define pointer to integer not pointer to array 
